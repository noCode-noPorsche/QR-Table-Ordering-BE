...
import {
  ...
  DishListRes,
  DishListResType,
  GetDishListWithPaginationQuery,
  GetDishListWithPaginationQueryType,
  GetDishListWithPaginationRes,
  GetDishListWithPaginationResType,
  DishParams,
  DishParamsType,
  DishRes,
  DishResType,
...
export default async function dishRoutes(fastify: FastifyInstance, options: FastifyPluginOptions) {
  ...
  fastify.get<{
    ...
    '/',
    {
      schema: {
        response: {
          200: GetDishListWithPaginationRes
        },
        querystring: GetDishListWithPaginationQuery
      }
    },
    async (request, reply) => {
      ...
      reply.send({
        data: {
          items: data.items as GetDishListWithPaginationResType['data']['items'],
