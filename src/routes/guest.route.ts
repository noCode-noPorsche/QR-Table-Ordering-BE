...
import { requireGuestHook, requireLoggedHook } from '@/hooks/auth.hooks'
import {
  LogoutBody,
  LogoutBodyType,
  RefreshTokenBody,
...
import { MessageRes, MessageResType } from '@/schemas/common.schema'
import {
...
export default async function guestRoutes(fastify: FastifyInstance, options: FastifyPluginOptions) {
  ...
  fastify.post<{
    ...
    '/orders',
    {
      ...
      preValidation: fastify.auth([requireLoggedHook, requireGuestHook])
    },
    async (request, reply) => {
